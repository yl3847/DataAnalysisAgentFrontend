 ...
 * <en/> Minimap plugin
 */
class Minimap extends base_plugin_1.BasePlugin {
    constructor(context, options) {
        ...
        this.onTransform = (0, util_1.throttle)(() => {
    ...
    setOnRender() {
        this.onRender = (0, util_1.debounce)(() => {
            this.renderMinimap();
            this.renderMask();
        }, this.options.delay);
    }
    bindEvents() {
        ...
        graph.on(constants_1.GraphEvent.AFTER_DRAW, this.onDraw);
    ...
    unbindEvents() {
        const { graph } = this.context;
        graph.off(constants_1.GraphEvent.AFTER_DRAW, this.onDraw);
    ...
    getElements() {
        ...
        const data = {
            nodes: originData.nodes.filter((node) => element === null || element === void 0 ? void 0 : element.getElement((0, id_1.idOf)(node))),
            edges: originData.edges.filter((edge) => {
                const edgeElement = element === null || element === void 0 ? void 0 : element.getElement((0, id_1.idOf)(edge));
                // 边数据存在且可见时才保留
                return edgeElement && (0, element_1.isVisible)(edgeElement);
            }),
            combos: originData.combos.filter((combo) => element === null || element === void 0 ? void 0 : element.getElement((0, id_1.idOf)(combo))),
        };
        if (!filter)
            return data;
    ...
    setShapes(canvas, data) {
        ...
        edges.forEach((datum) => canvas.appendChild(shape((0, id_1.idOf)(datum), 'edge')));
        combos.forEach((datum) => {
            canvas.appendChild(setPosition((0, id_1.idOf)(datum), shape((0, id_1.idOf)(datum), 'combo')));
        });
        nodes.forEach((datum) => {
            canvas.appendChild(setPosition((0, id_1.idOf)(datum), shape((0, id_1.idOf)(datum), 'node')));
        });
    }
    initCanvas() {
    ...
    get maskBBox() {
        ...
        const maskMin = this.canvas.canvas2Viewport((0, point_1.toPointObject)(canvasMin));
        const maskMax = this.canvas.canvas2Viewport((0, point_1.toPointObject)(canvasMax));
