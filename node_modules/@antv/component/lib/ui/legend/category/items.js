...
var CategoryItems = /** @class */ (function (_super) {
    tslib_1.__extends(CategoryItems, _super);
    function CategoryItems(options) {
        var _this = _super.call(this, options, {
            ...
            click: util_1.noop,
            mouseenter: util_1.noop,
            mouseleave: util_1.noop,
        }) || this;
    ...
    Object.defineProperty(CategoryItems.prototype, "renderData", {
        get: function () {
            var _a = this.attributes, data = _a.data, layout = _a.layout, poptip = _a.poptip;
            var style = (0, util_2.subStyleProps)(this.attributes, 'item');
            var d = data.map(function (datum, index) {
                var _a = datum.id, id = _a === void 0 ? index : _a, labelText = datum.label, valueText = datum.value;
                return {
                    id: "".concat(id),
                    index: index,
                    style: tslib_1.__assign({ layout: layout, labelText: labelText, valueText: valueText, poptip: poptip }, Object.fromEntries(Object.entries(style).map(function (_a) {
                        var _b = tslib_1.__read(_a, 2), key = _b[0], val = _b[1];
                        return [key, (0, util_2.getCallbackValue)(val, [datum, index, data])];
                    }))),
                };
            });
            return d;
    ...
    CategoryItems.prototype.getGridLayout = function () {
        ...
        var _b = tslib_1.__read(this.navigatorShape, 1), navWidth = _b[0];
        var _c = tslib_1.__read(this.grid, 2), gridRow = _c[0], gridCol = _c[1];
        ...
        return this.pageViews.children.map(function (item, index) {
            ...
            if (orientation === 'vertical')
                pos.reverse();
            var _c = tslib_1.__read(pos, 2), row = _c[0], col = _c[1];
            // calc x, y and shape
            ...
            var _d = tslib_1.__read([0, 0], 2), x = _d[0], y = _d[1];
            if (orientation === 'horizontal') {
                _a = tslib_1.__read([prevOffset, row * (rowHeight + rowPadding)], 2), x = _a[0], y = _a[1];
                prevOffset = col === gridCol - 1 ? 0 : prevOffset + colWidth + colPadding;
            }
            else {
                _b = tslib_1.__read([col * (colWidth + colPadding), prevOffset], 2), x = _b[0], y = _b[1];
    ...
    CategoryItems.prototype.getFlexLayout = function () {
        ...
        var _b = tslib_1.__read(this.navigatorShape, 1), navWidth = _b[0];
        var _c = tslib_1.__read(this.grid, 2), gridRow = _c[0], gridCol = _c[1];
        var _d = tslib_1.__read([maxWidth - navWidth, maxHeight], 2), limitWidth = _d[0], limitHeight = _d[1];
        var _e = tslib_1.__read([0, 0, 0, 0, 0, 0, 0, 0], 8), x = _e[0], y = _e[1], page = _e[2], pageIndex = _e[3], col = _e[4], row = _e[5], prevWidth = _e[6], prevHeight = _e[7];
        return this.pageViews.children.map(function (item, index) {
            var _a, _b, _c, _d;
            ...
            if (nextWidth <= limitWidth && ifSatisfied(col, function (c) { return c < gridCol; })) {
                _a = tslib_1.__read([prevWidth + colPadding, prevHeight, nextWidth], 3), x = _a[0], y = _a[1], prevWidth = _a[2];
                return { width: width, height: height, x: x, y: y, page: page, index: index, pageIndex: pageIndex++, row: row, col: col++ };
            }
            // wrap
            _b = tslib_1.__read([row + 1, 0, 0, prevHeight + height + rowPadding], 4), row = _b[0], col = _b[1], prevWidth = _b[2], prevHeight = _b[3];
            var nextHeight = prevHeight + height;
            if (nextHeight <= limitHeight && ifSatisfied(row, function (r) { return r < gridRow; })) {
                _c = tslib_1.__read([prevWidth, prevHeight, width], 3), x = _c[0], y = _c[1], prevWidth = _c[2];
                return { width: width, height: height, x: x, y: y, page: page, index: index, pageIndex: pageIndex++, row: row, col: col++ };
            }
            // paging
            _d = tslib_1.__read([0, 0, width, 0, page + 1, 0, 0, 0], 8), x = _d[0], y = _d[1], prevWidth = _d[2], prevHeight = _d[3], page = _d[4], pageIndex = _d[5], row = _d[6], col = _d[7];
            return { width: width, height: height, x: x, y: y, page: page, index: index, pageIndex: pageIndex++, row: row, col: col++ };
        });
    };
    ...
    CategoryItems.prototype.ifHorizontal = function (a, b) {
        ...
        return (0, utils_1.ifHorizontal)(orientation, a, b);
    };
    ...
    CategoryItems.prototype.renderItems = function (container) {
        ...
        (0, util_2.select)(container)
            ...
            return enter
                .append(function (_a) {
                var style = _a.style;
                return new item_1.CategoryItem({ style: style });
            })
    ...
    CategoryItems.prototype.relayoutNavigator = function () {
        ...
        var _c = tslib_1.__read(this.navigatorShape, 2), navWidth = _c[0], navHeight = _c[1];
    ...
    CategoryItems.prototype.adjustLayout = function () {
        var _this = this;
        var itemsLayouts = Object.entries((0, util_2.groupBy)(this.itemsLayout, 'page')).map(function (_a) {
            var _b = tslib_1.__read(_a, 2), page = _b[0], layouts = _b[1];
            return ({
                page: page,
                layouts: layouts,
            });
        });
        var categoryItems = tslib_1.__spreadArray([], tslib_1.__read(this.navigator.getContainer().children), false);
        itemsLayouts.forEach(function (_a) {
            var layouts = _a.layouts;
            var page = _this.pageViews.appendChild(new shapes_1.Group({ className: CLASS_NAMES.page.name }));
            layouts.forEach(function (layout) {
                ...
                (0, util_1.set)(item, '__layout__', layout);
    ...
    CategoryItems.prototype.renderNavigator = function (container) {
        var orientation = this.attributes.orientation;
        var navStyle = (0, util_2.subStyleProps)(this.attributes, 'nav');
        var style = (0, util_2.deepAssign)({ orientation: orientation }, navStyle);
        var that = this;
        container
            ...
            .join(function (enter) {
            return enter
                .append(function () { return new navigator_1.Navigator({ style: style }); })
